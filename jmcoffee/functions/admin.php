...
function jt_login_stylesheet() {
    ...
    wp_enqueue_style( 'jt-custom-login', get_template_directory_uri() . '/css/login.css', NULL, VERSION );
	wp_enqueue_script( 'jquery' );
	wp_enqueue_script( 'jt-custom-login', get_template_directory_uri() . '/js/login.js', NULL, VERSION );
...
function jt_admin_stylesheet() {
    ...
    wp_enqueue_style( 'jt-custom-admin', get_template_directory_uri() . '/css/admin.css', NULL, VERSION );
    wp_enqueue_script( 'jt-custom-admin-script', get_template_directory_uri() . '/js/admin.js', array('jquery'), VERSION, true);
...
function jt_page_dashboard_widget() {
	...
	add_meta_box(
        'jt_page_dashboard_widget',
        '최근 수정 페이지',
        'jt_page_dashboard_widget_function',
		'dashboard',
        'normal',
        'high'
    );
 ...
 * Custom admin dashboard widget revision
 * ************************************** */
function jt_inquiry_dashboard_widget() {

	add_meta_box(
        'jt_inquiry_dashboard_widget',
        '최근 문의내역',
        'jt_inquiry_dashboard_widget_function',
		'dashboard',
        'normal',
        'high'
    );
}
add_action( 'wp_dashboard_setup', 'jt_inquiry_dashboard_widget' );



function jt_inquiry_dashboard_widget_function () {

    global $wpdb;

    $table = $wpdb->prefix . 'inquiry';

    $sql        = $wpdb->prepare(
                    "
                        SELECT * FROM {$table} WHERE deleted=%s ORDER BY id DESC LIMIT %d
                    ",
                    '0',
                    10
                );
...
?>
    <table class="widefat" style="margin-bottom:5px;">
        ...
        <tbody>
            <?php foreach ( $list as $item ) { ?>
            <?php 
                $admin_posturl = admin_url( '/admin.php?page=jt-inquiry' );
            ?>
                <tr class="jt-dashboard-log-item">
                    <td class="jt-dashboard-log-item__title"><a href= "<?php echo $admin_posturl; ?>"><?php echo $item['content']; ?></a></td>
                    <td class="jt-dashboard-log-item__author"><?php echo $item['type']; ?></td>
                    <td class="jt-dashboard-log-item__date"><?php echo $item['created']; ?></td>
                </tr>
            <?php } ?>
        </tbody>
    </table>
 ...
 * Custom admin dashboard widget revision
 * ************************************** */
function jt_news_dashboard_widget() {
    
	add_meta_box(
        'jt_news_dashboard_widget',
        '최근 뉴스 게시글',
        'jt_news_dashboard_widget_function',
		'dashboard',
        'normal',
        'high'
    );
}
add_action( 'wp_dashboard_setup', 'jt_news_dashboard_widget' );



function jt_news_dashboard_widget_function () {

    global $wpdb;

    $sql = $wpdb->prepare(
        "
        SELECT * FROM {$wpdb->posts} WHERE post_type = %s AND post_status = %s ORDER BY post_modified DESC LIMIT %d
        ",
        'news',
        'publish',
        10

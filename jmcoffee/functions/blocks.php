...
function jt_blocks_remove_block_library_css(){
    if( !is_single() ){
        //wp_dequeue_style( 'global-styles' );           // 전역 스타일
...
function jt_blocks_editor_styles() {
    ...
    if( is_admin() ) {
        wp_enqueue_style( 'jt-blocks-editor-styles', get_theme_file_uri( '/css/admin-blocks.css' ), false, VERSION, 'all' );
    }
...
function jt_blocks_editor_script() {
    ...
    wp_enqueue_script( 'jt-admin-blocks-script', get_theme_file_uri( '/js/admin-blocks.js' ), array( 'wp-blocks' ), VERSION );
